    ...
    shows.showFromName(req.params.showname) //grab show info
        ...
        .then(() => {
               ...
               /* here we build the data into an array that Mustache can iterate over and
                ...
                /* ------------------------- just recommended artist : start ------------------------ */
                let similarArtistsList = require('fs').readFileSync('./views/similar-artists.mustache', 'utf8');


                let similarArtists = [];

                _relatedArtists.forEach( (relatedArtist, index) => {
                    _relatedArtists[index].slice(0, numberOfRows).forEach( nombre => {
                        similarArtists.push({
                            name: nombre.name,
                            source: _topPlayedArtists[index][0]
                        });
                    });
                });
                ...
                let renderedSimilarArists = Mustache.render( similarArtistsList , relatedView);


                /* ------------------------- just recommended artist : end ------------------------ */
                ...
                let renderedShowView = Mustache.render(showTemplate, { show: _show._name}, {
                    ...
                    'similar-artists': renderedSimilarArists
                });
